...
export async function sendFarmerOTP(rawMobile) {
  ...
  await pool.query(
    "INSERT INTO farmer_otps (mobile, code, expires_at) VALUES (?,?,?)",
    [mobile, code, expiresAt]
  );

  /* DEV-only provider */
  if (PROVIDER === "dev") {
    console.log(`[OTP][DEV][Farmer] ${mobile}: ${code} (expires ${EXPIRY_S}s)`);
  ...
  if (PROVIDER === "msg91_whatsapp") {
    ...
    if (!AUTH_KEY || !TEMPLATE_NAME || !NAMESPACE || !INTEGRATED_NO) {
      ...
      e.vendorStatus = 400;
      e.vendorBody = { reason: "Missing MSG91 config" };
      throw e;
    }
    ...
    // Variant A
    const bodyA = { body_1: { type: "text", value: code } };
    if (buttonVal.length <= 15) {
      bodyA.button_1 = { subtype: "url", type: "text", value: buttonVal };
    }
    const payloadA = {
      ...
      payload: {
        ...
        template: {
          ...
          namespace: NAMESPACE,
          to_and_components: [{ to: [to], components: bodyA }],
        },
      },
    };
    ...
    async function tryOnce(host, payload, variant) {
      ...
      if (resp.status >= 200 && resp.status < 300) {
        console.log(`[Farmer OTP] Sent via variant ${variant} @ ${host}`);
    ...
    console.error("[MSG91][Farmer OTP] last error:", {
      status: lastErr?.vendorStatus,
      host: lastErr?.host,
      variantTried: lastErr?.variant,
      body: lastErr?.vendorBody,
    });

    if (DEV_FALLBACK) {
      console.warn("[OTP][DEV_FALLBACK] returning DEV OTP:", code);
...
export async function verifyFarmerOTP(rawMobile, rawCode) {
  await ensureOtpTable();
  const mobile = normalizeMobile10(rawMobile);
  const code = String(rawCode || "").trim();

  const [rows] = await pool.query(
    "SELECT * FROM farmer_otps WHERE mobile=? AND code=? ORDER BY id DESC LIMIT 1",
    [mobile, code]
  );
